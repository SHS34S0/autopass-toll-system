...
--Morning (07:00-08:30) and Evening (15:30-17:00) -> Markup +20% (x1.2)
       CASE
           WHEN (CAST(strftime('%H', "passages"."passed_at") AS INTEGER) = 7) -- 07:00-07:59
               OR (CAST(strftime('%H', "passages"."passed_at") AS INTEGER) = 8 AND
                   CAST(strftime('%M', "passages"."passed_at") AS INTEGER) <= 30) -- 08:00-08:30
               OR (CAST(strftime('%H', "passages"."passed_at") AS INTEGER) = 15 AND
                   CAST(strftime('%M', "passages"."passed_at") AS INTEGER) >= 30) -- Норвезький пік часто з 15:30
       ...
       CAST(
               ROUND(
                       ("toll_stations"."base_price" * (100 - "vehicles"."discount_percent") /
                        100) * -- Базова зі знижкою
                       CASE
                           WHEN (CAST(strftime('%H', "passages"."passed_at") AS INTEGER) = 7) -- 07:00-07:59
                               OR (CAST(strftime('%H', "passages"."passed_at") AS INTEGER) = 8 AND
                                   CAST(strftime('%M', "passages"."passed_at") AS INTEGER) <= 30) -- 08:00-08:30
                               OR (CAST(strftime('%H', "passages"."passed_at") AS INTEGER) = 15 AND
                                   CAST(strftime('%M', "passages"."passed_at") AS INTEGER) >=
                                   30) -- Норвезький пік часто з 15:30
                               OR (CAST(strftime('%H', "passages"."passed_at") AS INTEGER) = 16)
                               THEN 1.2
                           ELSE 1.0
                           END
